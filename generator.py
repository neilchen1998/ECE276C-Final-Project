...
class VAE_Generator(Generator):
    ...
    def __init__(self):
        ...
        files = os.listdir(os.path.join(os.getcwd(), 'constraint-pi-4'))
        pretrained_file = 'pretrained.tar'
        if pretrained_file in files:
            checkpoint = torch.load(os.path.join(os.getcwd(), 'constraint-pi-4', pretrained_file))
            self.model.load_state_dict(checkpoint['model_state_dict'])
        else:
            self.dataset = np.load('dataset.npy')
            self.train(400,4096)
        self.samples = self.model.generate(np.random.normal(0,1,(1500,LATENT)))
    def generate(self,seed):
    ...
    def train(self,num_epochs,samples_per_batch):
        ...
        print('done training')
        torch.save({'model_state_dict':self.model.state_dict()},os.path.join(os.getcwd(),'constraint-pi-4/pretrained.tar'))
...
if __name__ == '__main__':
    ...
    dataset = np.zeros((2,))
    root = os.path.join(os.getcwd(),'constraint-pi-4')
    ...
    for i in range(NUM_NODES):
        ...
        outs = np.vstack([outs,out])
    elapsed = (time.time_ns()-start_t)/(10**9)
    # print('{:.2}'.format(elapsed))
    print(elapsed)
    plt.scatter(outs[:,0],outs[:,1])
    # plt.title('Nodes Generated by Baseline')
    plt.title('Nodes Generated by VAE (Hidden: {}, Latent: {})'.format(HIDDEN,LATENT,elapsed))
    plt.xlim(-3.5,3.5)
    plt.ylim(-3.5,3.5)
    plt.show()
